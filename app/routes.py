...
def nutriscore_form():
    ...
    try:
        ...
        data["Nutriscore"] = [{prediction_list[0]}]
        df = pd.DataFrame(data)
        formatted_datetime = date_time.strftime("%Y_%m_%d_%H_%M_%S")

        df.to_json(f"./save_data/data_{formatted_datetime}.json", orient='records', indent=4)
